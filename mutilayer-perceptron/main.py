...
def main():
    ...
    params = {
        "data_dir": os.path.join(filepath, "../taxi_data"), # directory where taxi data csv files are stored
        "save_dir": os.path.join(filepath, "./results"), # directory for storing processed data, predictions and plots
        "epochs": 5,
        "batch_size": 64,
        "learning_rate": 0.001,
...
def preprocess_data(params):
    ...
    df_taxi = spark.read.option("header", True).csv(os.path.join(data_dir, "green_tripdata_2021-01.csv"))
    ...
    # save processed data
    df_train_scaled.repartition(1).write.parquet(os.path.join(save_dir,"train_data"), mode="overwrite")
    df_test_scaled.repartition(1).write.parquet(os.path.join(save_dir,"test_data"), mode="overwrite")
...
class MLP(Model):
    def __init__(self, params):
        ...
        self.checkpoint_filepath = os.path.join(self.save_dir, 'MLP_checkpoints_keras/chk')
    ...
    def build_MLP(self):
        ...
        with strategy.scope():
            ...
            Dense(self.hidden_units, input_shape=(self.train_x.shape[1],), activation='relu'),
    ...
    def train_model(self):
        # train model
        print('MODEL SUMMARY: \n', self.model.summary())
        csv_logger = CSVLogger(os.path.join(self.save_dir, 'train_history.csv'), separator=',', append=False)
        cb_chk   = ModelCheckpoint  (monitor='val_loss', save_best_only=True, filepath=self.checkpoint_filepath)
